...
// check is verified reviewer
if ($_SESSION['client']['isReviewer'] == 0) {
    $_SESSION['error'] = array();
    $_SESSION['error'][] = 'First you have to be a verified reviewer! Give this Quiz to a verified reviewer.';
    header("location: quiz.php");
    exit();
}

// retireving review request id
if (!empty($_GET)) {
    if (isset($_GET['id'])) {
        $rr_id = $_GET['id'];
...
$query_quiz = "SELECT * FROM `review_request_quiz` where `review_request_id` = '$rr_id'";
$result_quiz = mysqli_query($connection_database, $query_quiz);
$questions = array();
if ($result_quiz->num_rows == 0) {
    header("location: book_review.php?id=".$rr_id."&is_r_r=1");
    exit();
}
...
if ($current_question >= count($questions)) {
    ...
    if ($obtain >= 100) {
        $_SESSION['message']['success'] = 'Congratulations! You can now review the book. Score ' . $obtain . '%';
        ...
        $_SESSION['error'] = array();
        $_SESSION['error'][] = 'Sorry! You have failed the Questions. ScoreS ' . $obtain . '%';
        unset($_SESSION['question_number']); // Reset the question number when quiz is completed
        unset($_SESSION['score']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    header('Location: review_request_quiz.php'); // Reload for the next question
    exit();
}
?>
...
<div class="container mt-5 pt-5 min-vh-100">
    <div class="card mt-5 pt-5">
        ...
        <div class="card-body">
            ...
            <form id="quiz-form" method="POST" action="review_request_quiz.php">
